...
9.位运算符：(二进制对位转换)
    1.& 与运算：(有0则0)
    	例： int a = 1;	//001
			 int b = 5;	 //101
			 Console.WriteLine(a & b);	//二进制为:001	则打印为:1
	2.| 或运算：(有1则1)
        例：  int a = 1;	//001
			 int b = 5;	 //101
			 Console.WriteLine(a & b);	//二进制为:101	则打印为:5
	3.^ 异或运算：(相同为0 不同为1)
        例：	int a = 1; //001
			 int b = 5;	 //101
			 Console.WriteLine(a & b);	//二进制为:100	则打印为:4
	4.~ 取反运算：(0变为1 1变为0)
        例：	int a = 1; //0000 0000 0000 0000 0000 0000 0000 0001
			 Console.WriteLine(~b); 	
			     //二进制为:1111 1111 1111 1111 1111 1111 1111 1110
				 //则打印为: -14	(前面的1为反码补码)
	5.<< 左移运算：(右侧加0)
         例：	int a = 5;	//101
				c = a << 3;	//右侧加上3个0: 101000
			 Console.WriteLine(c);	//打印为40
	6.>> 右移运算：(右侧去数)
         例： int a = 5;	//101
				c = a >> 2;	//右侧去除2个数字: 1
			 Console.WriteLine(c);	//打印为1
```

```c#
10.三位运算符：(3个空位 2个符号)
    1.格式：空位(bool变量类型)    ?空位    :空位    ;
	2.例： int a = 5;
		   string str = a < 1 ? "a大于1" : "a不满足条件";	(返回值要与前面的变量一致)
           Console.WriteLine(str);	//打印为: a不满足条件
```
